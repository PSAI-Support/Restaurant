    ...
    class Report extends PosComponent {
        ...
        async showReports() {
            ...
            if (this.env.pos.config.report_product_summary) {
                list_report.push({
                    'id': 1, 'name': 'Report Products Summary', 'item': 1
                })
            }
            if (this.env.pos.config.report_order_summary) {
                list_report.push({
                    'id': 2, 'name': 'Report Orders Summary', 'item': 2
                })
            }
            if (this.env.pos.config.report_payment_summary) {
                list_report.push({
                    'id': 3, 'name': 'Payments Summary', 'item': 3
                })
            }
            if (this.env.pos.config.report_sale_summary) {
                list_report.push({
                    'id': 4, 'name': 'Z-Report (Your Session Sale Summary)', 'item': 4
                })
            }
            list_report.push({
                'id': 5, 'name': 'Sale Summary Detail of your Session', 'item': 5
            })
            var to_date = new Date().toISOString().split('T')[0];
            var date = new Date();
            var firstDay = new Date(date.getFullYear(), date.getMonth(), 1);
            var from_date = firstDay.toISOString().split('T')[0];
            let {confirmed, payload: selectedReports} = await this.showPopup('PopUpSelectionBox', {
                title: this.env._t('Select the report'), items: list_report, onlySelectOne: true,
            });
                    ...
                    if (report_id == 4) {
                        ...
                        if (this.env.pos.proxy.printer) {
                            ...
                            this.showScreen('ReportScreen', {
                                report_html: reportHtml, report_xml: reportXml
                            })
                        }
                    }
                    if (report_id == 5) {
                        ...
                        if (this.env.pos.proxy.printer) {
                            ...
                            this.showScreen('ReportScreen', {
                                report_html: report_html, report_xml: report_xml
                            })
                        }
                    }
                }
        ...
        async buildProductsSummaryReport(values) {
            ...
            if (values.current_session_report) {
                val = {
                    'from_date': null, 'to_date': null, 'summary': summary, 'session_id': this.env.pos.pos_session.id,
                };
            } else {
                if (!values.from_date || !values.to_date) {
                    return this.env.pos.alert_message({
                        title: this.env._t('Error'), body: this.env._t('From or To Date is missed, required input')
                    })
                }
                val = {
                    'from_date': values.from_date, 'to_date': values.to_date, 'summary': summary
                };
            }
            let params = {
                model: 'pos.order', method: 'product_summary_report', args: [val],
            };
        ...
        renderProductsSummaryReport(values, results) {
            if (Object.keys(results['category_summary']).length == 0 && Object.keys(results['product_summary']).length == 0 && Object.keys(results['location_summary']).length == 0 && Object.keys(results['payment_summary']).length == 0) {
                return this.env.pos.alert_message({
                    title: this.env._t('Warning'), body: this.env._t('Data not found for report')
                })
            } else {
        ...
        async buildOrdersSummaryReport(values) {
            ...
            let params = {
                model: 'pos.order', method: 'order_summary_report', args: [val],
            };
            let results = await this.rpc(params).then(function (result) {
                return result
            }, function (err) {
        ...
        renderOrdersSummaryReport(values, results) {
            ...
            if (results) {
                if (Object.keys(results['category_report']).length == 0 && Object.keys(results['order_report']).length == 0 && Object.keys(results['payment_report']).length == 0) {
                    return this.env.pos.alert_message({
                        title: this.env._t('Warning'), body: this.env._t('Data not found for report')
                    })
                } else {
        ...
        async buildPaymentsSummaryReport(values) {
            ...
            if (values.current_session_report) {
                val = {
                    'summary': summary, 'session_id': this.env.pos.pos_session.id,
                };
            } else {
                if (!values.from_date || !values.to_date) {
                    return this.env.pos.alert_message({
            ...
            let params = {
                model: 'pos.order', method: 'payment_summary_report', args: [val],
            };
            let results = await this.rpc(params).then(function (result) {
                return result
            }, function (err) {
