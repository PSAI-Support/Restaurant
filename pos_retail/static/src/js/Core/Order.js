...
odoo.define('pos_retail.order', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        async _setOrdertoCancel(remove_reason) {
            const order_ids = await rpc.query({
                model: 'pos.order',
                method: 'search',
                args: [[['pos_reference', '=', this.name], ['session_id', '=', this.pos.pos_session.id], ['state', '=', 'draft']]],
                kwargs: {},
            ...
            if (order_ids && order_ids.length) {
                await rpc.query({
                    model: 'pos.order', method: 'write', args: [order_ids, {
                        'remove_reason': remove_reason, 'state': 'cancel'
                    }],
                })
            }
        },
        ...
        get_payment_total_by_method_id() {
            let result = {}
            for (let i = 0; i <= this.paymentlines.models.length; i++) {
                let payment = this.paymentlines.models[i]
                if (payment && payment.payment_method) {
                    if (!result[payment.payment_method.id]) {
                        result[payment.payment_method.id] = payment.amount
                    } else {
                        result[payment.payment_method.id] += payment.amount
                    }
                }
        ...
        add_paymentline: function (payment_method) {
            ...
            if (payment_method.journal && payment_method.journal.pos_method_type == 'credit' && !this.pos.config.credit) {
                if (!client) {
                    this.remove_paymentline(newPaymentline)
                    Gui.showPopup('ErrorPopup', {
                        title: this.pos.env._t('Error'),
                        body: this.pos.env._t('Credit method only available for customer have credit Amount, current Order null customer !')
                    })
                    return true
                } else {
                    if (client.balance <= 0) {
                        ...
                        Gui.showPopup('ErrorPopup', {
                            title: this.pos.env._t('Error'),
                            body: this.pos.env._t('Customer have not credit Amount for payment !')
                        })
                        return true
                    } else {
                        paymentLinesRemove = []
                        for (let i = 0; i < this.paymentlines.models.length; i++) {
                            let paymentLine = this.paymentlines.models[i]
                            if (paymentLine.cid != newPaymentline.cid) {
                                paymentLinesRemove.push(paymentLine)
                            }
                        }
                        for (let i = 0; i < paymentLinesRemove.length; i++) {
                            let paymentLine = paymentLinesRemove[i]
                            this.remove_paymentline(paymentLine)
                        }

                        newPaymentline.set_amount(0)
                        const due_amount = this.get_due()
                        if (client.balance >= due_amount) {
                            newPaymentline.set_amount(this.get_due())
                        } else {
                            newPaymentline.set_amount(client.balance)
                        }
        ...
        set_client: async function (client) {
            ...
            if (this.pos.config.wallet) {
                this.trigger('automatic-wallet-payment')
            }
            if (this.pos.config.credit) {
                this.trigger('automatic-credit-payment')
            }
            let paymentLineNeedRemove = []
            this.paymentlines.models.forEach(p => {
                if (p.payment_method && p.payment_method.journal && (p.payment_method.journal.pos_method_type == 'credit' || p.payment_method.journal.pos_method_type == 'wallet')) {
                    paymentLineNeedRemove.push(p)
                }
            })
            paymentLineNeedRemove.forEach(p => self.remove_paymentline(p))
            const end = Date.now();
    ...
    models.Orderline = models.Orderline.extend({
        ...
        init_from_JSON: function (json) {
            ...
            if (json.moved) {
                this.moved = json.moved
            }
            if (json.partner_ids) {
                this.partner_ids = json.partner_ids
            }
        },
        ...
        export_as_JSON: function () {
            ...
            if (this.partner_ids) {
                json.partner_ids = this.partner_ids
            }
            return json;
        },
