odoo.define('pos_retail.PaymentScreen', function (require) {
    ...
    const RetailPaymentScreen = (PaymentScreen) => class extends PaymentScreen {
        ...
        mounted() {
            ...
            if (this.env.pos.config.wallet) {
                this._currentOrder.trigger('automatic-wallet-payment')
            }
            if (this.env.pos.config.credit) {
                this._currentOrder.trigger('automatic-credit-payment')
            }
        }

        get itemsNumber() {
            let items_number = this.currentOrder ? this.currentOrder.orderlines.reduce((items_number, line) => items_number + line.quantity, 0) : 0;
            return items_number
        }
        ...
        async validateOrder(isForceValidate) {
            ...
            // todo: check credit
            if (this._currentOrder && this._currentOrder.get_client() && this._currentOrder.get_client().balance > 0) {
                let paymentIsCredit = this._currentOrder.paymentlines.models.find(p => p.payment_method && p.payment_method.journal && p.payment_method.journal.pos_method_type == 'credit')
                if (paymentIsCredit) {
                    let allPaymentByMethodId = this._currentOrder.get_payment_total_by_method_id()
                    if (allPaymentByMethodId[paymentIsCredit.payment_method.id] > this._currentOrder.get_client().balance) {
                        this.showPopup('ErrorPopup', {
                            title: this.env._t('Error'),
                            body: this.env._t('Register Payment with Credit bigger than Credit of Customer !'),
                        })
                        return
                    }
            ...
            if (await this._isOrderValid(isForceValidate) && this.env.pos.config.quickly_checkout_bill) {
