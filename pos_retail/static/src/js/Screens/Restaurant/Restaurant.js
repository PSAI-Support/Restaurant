odoo.define('pos_retail.restaurant', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        get_orders_by_table: function (table) {
            let orders = this.db.get_unpaid_orders()
            orders = orders.filter(o => o.table_id && o.table_id == table.id && o.pos_session_id == this.pos_session.id)
            return orders;
        },

        _getStyleOfReceipt() {
            if (this.config.receipt_fix && this.config.receipt_fix_width > 0) {
                let styleReceipt = "width: " + this.config.receipt_fix_width + "mm; "
                styleReceipt += "margin: " + this.config.receipt_fix_margin + "mm; "
                styleReceipt += "margin-left: " + this.config.receipt_fix_margin_left + "mm; "
                styleReceipt += "margin-right: " + this.config.receipt_fix_margin_right + "mm; "
                styleReceipt += "font-size: " + this.config.receipt_fix_font_size + "px; "
                return styleReceipt
            } else {
                return ""
            }
        ...
        async transfer_order_to_table(table) {
            const currenTable = this.order_to_transfer_to_different_table.table
            const res = _super_posmodel.transfer_order_to_table.apply(this, arguments);
            if (currenTable && table && currenTable.id != table.id) {
                ...
                for (let i = 0; i < printers.length; i++) {
                    ...
                    let receipt = QWeb.render('TableTransfer', {
                        pos: this,
                        user: this.user.name,
                        floor_from: currenTable.floor.name,
                        table_from: currenTable.name,
                        floor_to: table.floor.name,
                        table_to: table.name,
                        hours: hours,
                        minutes: minutes,
                    })
                    const printStatus = await printer.print_receipt(receipt)
