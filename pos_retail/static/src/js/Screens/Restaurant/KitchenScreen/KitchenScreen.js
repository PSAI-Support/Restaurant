odoo.define('pos_retail.KitchenScreen', function (require) {
    ...
    class KitchenScreen extends IndependentToOrderScreen {
        ...
        get orderList() {
            ...
            // -----------------------------------
            if (this.env.pos.config.screen_type == 'kitchen' && !this.env.pos.config.display_all_product) {
            ...
            if (this.env.pos.config.sort_by_ticket == 'request_time_desc') {
                orders = orders.sort(this.env.pos.sort_by('ticket_number', true, parseInt))
            }
            if(orders.length > this.env.pos.config.maximum_ticket_display) {
                orders = orders.slice(0, this.env.pos.config.maximum_ticket_display)
            }
            return orders
        }
