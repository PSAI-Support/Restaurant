odoo.define('pos_retail.ProductScreenCenterPaneFeaturesButtons', function (require) {
    'use strict';
    ...
    class ProductScreenCenterPaneFeaturesButtons extends PosComponent {
        constructor() {
            super(...arguments);
        ...
        async partialPaymentOrder() {
            ...
            if (confirmed) {
                ...
                if (!selectedOrder.get_client()) {
                    ...
                    if (confirmed) {
                        ...
                        return this.env.pos.alert_message({
                            title: this.env._t('Error'), body: this.env._t('Required set Customer for Partial Order')
                        })
                    }
                }
                ...
                let {confirmed, payload: paymentMethod} = await this.showPopup('SelectionPopup', {
                    title: this.env._t('Are you want Register one Part Amount of Total Order'), list: lists
                })
                if (confirmed) {
                    let {confirmed, payload: number} = await this.showPopup('NumberPopup', {
                        title: this.env._t('How much Customer need Register Payment ?'), startingValue: 0
                    })
                    if (confirmed) {
                        number = parseFloat(number)
                        if (number < 0) {
                            return this.showPopup('ErrorPopup', {
                                title: this.env._t('Error'), body: this.env._t('Register Amount required Bigger than 0')
                            })
                        }
        ...
        async sendReceiptViaWhatsApp() {
            ...
            let {confirmed, payload: messageNeedSend} = await this.showPopup('TextAreaPopup', {
                title: this.env._t('What message need to send Client ?'), startingValue: message
            })
            if (confirmed) {
            ...
            if (mobile_no) {
                ...
                if (confirmed) {
                    ...
                    let {confirmed, payload: number} = await this.showPopup('NumberPopup', {
                        title: this.env._t("Please input WhatsApp Client's Mobile ?"), startingValue: 0
                    })
                    if (confirmed) {
                        mobile_no = number
                        let responseOfWhatsApp = await this.rpc({
        ...
        async setServicesOrder() {
            ...
            const list = this.env.pos.services_charge.map(service => ({
                id: service.id, name: service.name, item: service
            }))
            let {confirmed, payload: selectedItems} = await this.showPopup('PopUpSelectionBox', {
                title: this.env._t('Please select one Service need add to Order'), items: list, onlySelectOne: true,
            });
        ...
        async signatureOrder() {
            ...
            if (confirmed) {
                if (!values.signature) {
                    return this.env.pos.alert_message({
                        title: this.env._t('Warning'), body: this.env._t('Signature not success, please try again')
                    })
                } else {
        ...
        async setGuests() {
            const {confirmed, payload: inputNumber} = await this.showPopup('NumberPopup', {
                startingValue: this.nGuests, cheap: true, title: this.env._t('Guests ?'),
            });
        ...
        async lockTable() {
            ...
            let {confirmed, payload: selection} = await this.showPopup('SelectionPopup', {
                title: this.env._t('Selection Lock Type'), list: [{
                    label: this.env._t('Only Lock Selected Order'), item: true, id: 1,
                }, {
                    label: this.env._t('Lock all Orders have Items in Cart'), item: false, id: 2,
                }],
            })
            if (confirmed) {
                if (selection) {
        ...
        async selectLoyaltyReward() {
            ...
            if (!client) {
                const {
                    confirmed,
                    payload: newClient
                } = await this.env.pos.chrome.showTempScreen('ClientListScreen', {
                    client: null,
                    body: this.env._t('Required Customer')
                });
                if (confirmed) {
        ...
        async printReceipt() {
            ...
            await orderReceipt.mount(fixture);
            const report_html = orderReceipt.el.outerHTML;
            if (this.env.pos.proxy.printer) {
                this.env.pos.proxy.printer.print_receipt(report_html)
            } else {
                this.showScreen('ReportScreen', {
                    report_html: report_html, report_xml: null
                })
            }
        }
        ...
        async sendInput(key) {
            ...
            if (key == 'GlobalDisc') {
                ...
                const list = this.env.pos.discounts.map(discount => ({
                    id: discount.id, name: discount.name, item: discount,
                }))
                let {confirmed, payload: selectedItems} = await this.showPopup('PopUpSelectionBox', {
            ...
            if (key == 'SetNotes') {
                const {confirmed, payload: note} = await this.showPopup('TextAreaPopup', {
                    title: this.env._t('Set Notes to Order'), startingValue: selectedOrder.get_note()
                })
                if (confirmed) {
            ...
            if (key == 'onOfInvoice') {
                ...
                if (selectedOrder.is_to_invoice() && !selectedOrder.get_client()) {
                    ...
                    if (confirmed) {
                        this.env.pos.alert_message({
                            title: this.env._t('Order to Invoice is ON'), body: this.env._t('Order will create Invoice')
                        })
                        selectedOrder.set_client(newClient);
                    } else {
                        this.env.pos.alert_message({
            ...
            if (key == 'setStartCategory') {
                const pos_categories = this.env.pos.pos_categories.map(categ => ({
                    id: categ.id, label: categ.name, isSelected: false, item: categ
                }))
                pos_categories.push({
                    id: 0, label: this.env._t('All Categories'), isSelected: false, item: null
                })
                    ...
                    if (categ) {
                        await this.rpc({
                            model: 'pos.config', method: 'write', args: [[this.env.pos.config.id], {
                                'start_category': true, 'iface_start_categ_id': categ.id
                            }],
                        })
                    } else {
                        await this.rpc({
                            model: 'pos.config', method: 'write', args: [[this.env.pos.config.id], {
                                'start_category': false, 'iface_start_categ_id': null
                            }],
                        })
                    }
            ...
            if (key == 'setSeller') {
                ...
                if (!order || !order.get_selected_orderline()) {
                    return this.env.pos.alert_message({
                        title: this.env._t('Error'), body: this.env._t('Your order is blank cart')
                    })
                }
                const list = this.env.pos.sellers.map(seller => ({
                    id: seller.id, label: seller.name, isSelected: false, item: seller
                }))
            ...
            if (key == 'AutoRecommendations') {
                ...
                if (product_recommendation) {
                    let {confirmed, payload: count} = await this.showPopup('NumberPopup', {
                        title: this.env._t('How many products will display for Recomment ?'), startingValue: 20
                    })
                    if (confirmed) {
        ...
        async cashInOut() {
            ...
            if (sessions.length) {
                ...
                let {confirmed, payload: values} = await this.showPopup('CashSession', {
                    title: this.env._t('Take Cash In/Out of Your Session'), session: sessionSelected
                })
                if (confirmed) {
                    ...
                    if (action == 'setClosingBalance' && values.value.length > 0) {
                        ...
                        this.env.pos.alert_message({
                            title: this.env._t('Successfully'), body: this.env._t('Set Closing Balance !')
                        })
                        return this.cashInOut()
                    }
                }
        ...
        async changeToReturnMode() {
            ...
            if (selectedOrder.orderlines.models.length <= 0) {
                return this.env.pos.alert_message({
                    title: this.env._t('Error'), body: this.env._t('Your shopping cart is empty')
                })
            }
    ...
    ProductScreenCenterPaneFeaturesButtons.template = 'ProductScreenCenterPaneFeaturesButtons';

    Registries.Component.add(ProductScreenCenterPaneFeaturesButtons);

    return ProductScreenCenterPaneFeaturesButtons;
});
