odoo.define('pos_retail.ReceiptScreen', function (require) {
    ...
    const RetailReceiptScreen = (ReceiptScreen) => class extends ReceiptScreen {
        constructor() {
            super(...arguments);
            NumberBuffer.use({
        ...
        async _saveReceipt() {
            try {
                ...
                await this.rpc({
                    model: 'pos.order', method: 'saveReceipt', args: [[], order_server_id, ticketImage],
                });
        ...
        async downloadInvoice() {
            ...
            if (order_ids.length == 1) {
                        ...
                        if (!backendOrder.partner_id) {
                            ...
                            if (confirmed) {
                                ...
                                if (confirmed) {
                                    ...
                                    await this.rpc({
                                        model: 'pos.order', method: 'write', args: [[backendOrder.id], {
                                    ...
                                    let order_ids = await this.rpc({
                                        model: 'pos.order',
                                        method: 'search_read',
                                        domain: [['id', '=', backendOrder.id]],
                                        fields: ['id', 'account_move']
                                    })
                                    backendOrder = order_ids[0]
                                    await this.env.pos.do_action('acccount.account_invoices', {
                                        additional_context: {
                                            active_ids: [backendOrder.account_move[0]],
                                        }
                                    })
                                }
                            ...
                            if (!backendOrder.account_move) {
                                await this.rpc({
                                    model: 'pos.order', method: 'action_pos_order_invoice', args: [[backendOrder.id]],
                                })
                            } else {
                                await this.env.pos.do_action('acccount.account_invoices', {
                                    additional_context: {
                ...
                this.env.pos.alert_message({
                    title: this.env._t('Error'), body: this.env._t('Order has Duplicate. We can not print the Invoice')
                })
            }
        }
    }
