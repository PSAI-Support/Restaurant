...
class pos_controller(PosController):
    ...
    def pos_web(self, config_id=False, **k):
        ...
        session_info['model_ids']['res.partner']['count'] = count_partners[0][0] if len(count_partners) == 1 else 0
        ...
        session_info['iot_installed'] = None
        if request.env['ir.module.module'].sudo().search([
            ('name', '=', 'pos_iot'),
            ('state', '=', 'installed'),
        ],
                limit=1):
            session_info['iot_installed'] = True
