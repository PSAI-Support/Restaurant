...
class PosConfig(models.Model):
    ...
    quickly_checkout_bill = fields.Boolean('Allow quickly Complete Order and get bill')
    ...
    load_direct = fields.Boolean("Active Load Direct")

    load_direct_categ_ids = fields.Many2many(
        'pos.category',
        'pos_config_load_direct_categ_rel',
        'config_id',
        'categ_id',
        string='POS Categories allow POS Cashier select',
        help='Define POS categories here \n'
             'and POS cashier can select categories and change Pos Categories Screen'
    )

    load_direct_floor_ids = fields.Many2many(
        'restaurant.floor',
        'pos_config_load_direct_restaurant_floor_rel',
        'config_id',
        'floor_id',
        string='Restaurant Floors allow POS Cashier select'
    )
    ...
    def _get_forbidden_change_fields(self):
        context = self._context.copy()
        if context.get('update_from_pos', None):
            return []
        else:
            return super(PosConfig, self)._get_forbidden_change_fields()
    ...
    def open_ui(self):
        ...
        try:
            ...
            _logger.error('Have issue when open_ui: %s' % ex)
    ...
    def open_session_cb(self, check_coa=True):
        ...
        try:
            ...
            _logger.error('Have issue when open_session_cb: %s' % ex)
