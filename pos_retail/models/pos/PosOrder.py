...
class POSOrder(models.Model):
    ...
    partner_ids = fields.Many2many('res.partner', 'pos_order_partner_rel', 'order_id', 'partner_id', string='Customers in Table')
        ...
        for order in self:
            ...
            if vals.get('state', False) in ['paid', 'invoiced']:
                ...
                for payment in order.payment_ids:
                    ...
                    credits_existed = self.env['res.partner.credit'].search([
                        ('payment_id', '=', payment.id)
                    ])
                    if payment_method.journal_id and order.partner_id and payment_method.journal_id.pos_method_type == 'credit' and len(credits_existed) == 0:
                    ...
                    if payment_method.journal_id and payment_method.journal_id.pos_method_type == 'voucher' and payment.voucher_id:
    ...
    def _process_order(self, order, draft, existing_order):
        ...
        if draft:
            self.browse(order_id).write({
                'order_json': base64.encodebytes(
                    json.dumps(order['data'], default=date_utils.json_default).encode('utf-8')),
            })
