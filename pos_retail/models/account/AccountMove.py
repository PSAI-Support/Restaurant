...
class AccountMove(models.Model):
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        if context.get('pos_config_id', None):
            ...
            if config.load_invoices_limit > 0:
                limit = config.load_invoices_limit
            else:
                limit = 100
            order = 'id DESC'
    ...
    def write(self, vals):
        ...
        for account_move in self:
            credit_exist = credit_object.search([('move_id', '=', account_move.id)])
            if account_move.add_credit and vals.get('state', None) == 'posted':
                if not credit_exist:
            ...
            if vals.get('state', None) in ['draft', 'cancel']:
                credit_object.search([('move_id', '=', account_move.id)]).write({'active': False})
            if vals.get('state', None) == 'posted':
                _logger.info('[Account Move %s] posting' % account_move.id)
